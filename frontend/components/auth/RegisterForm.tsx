...
import { Eye, Factory, LockKeyhole, Mail, Phone, User } from "lucide-react";
  ...
  const handleRegister = async () => {
    try {
      ...
      if (error.response?.status === 422) {
        setToast({
          open: true,
          message: error.response?.data?.detail?.[0]?.msg.replace(
            "Value error, ",
            "",
          ),
          type: "error",
        ...
        setToast({
          open: true,
          message:
            error.response?.data?.detail.replace("Value error, ", "") ||
    ...
    <div className="bg-[#090909] h-screen flex">
      <div className="mx-auto w-150 my-auto relative">
        <div className="flex flex-col border-stone-700/20 border rounded-3xl px-10 pt-10 pb-20 relative overflow-hidden bg-[#0d0d0f]">
          <div className="absolute right-0 top-1/2 -translate-y-1/2 h-3/4 w-px bg-linear-to-b from-transparent via-blue-500 to-transparent" />
          <div className="absolute left-0 top-1/2 -translate-y-1/2 h-3/4 w-px bg-linear-to-b from-transparent via-blue-500 to-transparent" />
          <div className="bg-blue-500/50 w-40 h-100 rounded-full absolute top-1/2 -translate-y-1/2 -left-30 blur-3xl z-10" />
          ...
          <h2 className="text-white text-5xl w-fit mx-auto mb-2">Register</h2>
          <p className="text-stone-500 text-center mt-5">
            Register your account and seamlessly managing your projects, ideas,
            and progress.
          </p>
          ...
          {/* Email field */}
          <div className="mb-5 border border-stone-700 rounded-full relative">
            <svg
              className="absolute inset-0 w-full h-full pointer-events-none"
              xmlns="http://www.w3.org/2000/svg"
            >
          ...
          <div className="mb-5 border border-stone-700 rounded-full relative">
            ...
            />
            <Eye
              className="text-stone-600 absolute top-1/2 -translate-y-1/2 right-4 cursor-pointer hover:scale-120"
              size={18}
              onClick={() => setHide1((prev) => !prev)}
            />
          </div>

          {/* Confrim_Password field */}
          <div className="mb-5 border border-stone-700 rounded-full relative">
            <svg
              className="absolute inset-0 w-full h-full pointer-events-none"
              xmlns="http://www.w3.org/2000/svg"
          ...
          {/* Company_name field */}
          <div className="mb-15 border border-stone-700 rounded-full relative">
            <svg
              className="absolute inset-0 w-full h-full pointer-events-none"
              xmlns="http://www.w3.org/2000/svg"
            >
          ...
          <motion.button
            ...
            className="text-center text-white text-lg border border-l-blue-500/60 border-stone-700 py-3 rounded-full bg-stone-500/20 hover:bg-stone-400/20 transition-colors cursor-pointer"
          >
            Register
          </motion.button>
